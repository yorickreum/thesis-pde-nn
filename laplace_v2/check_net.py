...
def solution(x: torch.tensor, y: torch.tensor):
    return (1 / (torch.exp(laplace_v2.PI_TENSOR) - torch.exp(-laplace_v2.PI_TENSOR))) * torch.sin(
        laplace_v2.PI_TENSOR * x) * (
                   torch.exp(laplace_v2.PI_TENSOR * y) - torch.exp(- laplace_v2.PI_TENSOR * y))
...
def check_net(net):
    ...
    for ix in range(steps):
        for iy in range(steps):
            x = torch.tensor(lin_space[ix], dtype=laplace_v2.DTYPE)
            y = torch.tensor(lin_space[iy], dtype=laplace_v2.DTYPE)
            netxy = net(torch.tensor([x, y], dtype=laplace_v2.DTYPE, device=device)).cpu()
            zNet[iy][ix] = netxy
