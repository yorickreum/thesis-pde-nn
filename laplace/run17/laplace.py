...
def loss_func(input, prediction):
    ...
    ones = torch.unsqueeze(torch.ones(len(input)), 1)
...
ts_term1 = torch.tensor(ts_term1, dtype=torch.float, requires_grad=True)
ts_term2 = torch.tensor(ts_term2, dtype=torch.float, requires_grad=True)
