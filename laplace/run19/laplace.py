...
optimizer = torch.optim.LBFGS(net.parameters(), lr=.005)  # adagrad, lr_decay=.005
...
def loss_func(input, prediction):
    ...
    ones = torch.unsqueeze(torch.ones(len(input), dtype=torch.float, device=device), 1)
...
for r in itertools.product(x, y):
    xyPerms += [[r[0], r[1]]]
xyPerms = torch.tensor(xyPerms, dtype=torch.float, requires_grad=True, device=device)
...
ts_term1 = torch.tensor(ts_term1, dtype=torch.float, requires_grad=True, device=device)
ts_term2 = torch.tensor(ts_term2, dtype=torch.float, requires_grad=True, device=device)
