...
class PressureHeadNet(nn.Module):
    ...
    def forward(self, x):
        x = torch.sigmoid(self.hidden1(x))  # tanh activation function for first hidden layer
        x = torch.sigmoid(self.hidden2(x))  # linear activation function for second layer @TODO why? okay like this?
