...
def objective(trial):
    ...
    model = Model(
        n_hidden=trial.suggest_int("n_hidden_", 1, 16),
        n_neurons=trial.suggest_int("n_neurons", 4, 32),
        device=device
    )
    ...
    used_gpus = json.loads((os.getenv("YORICK_USED_GPUS", "[]")))
    used_gpus = [i for i in used_gpus if i != gpu_to_use]
    os.environ["YORICK_USED_GPUS"] = json.dumps(used_gpus)
