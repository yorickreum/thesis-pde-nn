...
import oscillation
from oscillation.hyper import objective
from oscillation.model import Model

def train():
    # main training loop
    model = Model(n_hidden=oscillation.HIDDEN, n_neurons=oscillation.NEURONS)
    parameters = model.h_net.parameters()
    optimizer = torch.optim.Adam(parameters, lr=oscillation.LR)

    startEpoches = time.time()
    for i in range(int(oscillation.EPOCHS)):
        optimizer.zero_grad()  # clear gradients for next train
        loss = model.loss_func(input=model.tz_pairs)
        with torch.no_grad():
            if i % oscillation.LOGSTEPS == 0:
                print("step " + str(i) + ": ")
                print(loss)
        ...
        with torch.no_grad():
            if i % 10 == 0:
                if len(model.h_net.losses) >= 11:
                    ...
                    if all(llc <= torch.finfo(oscillation.DTYPE).eps for llc in last_loss_changes):
...
def hyperstudy():
    ...
    study.optimize(
        objective,
        n_trials=oscillation.HYPERTRIALS,
        n_jobs=n_jobs,
        # timeout=(10 * 60),  # 2 h,
