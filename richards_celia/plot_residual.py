...
from richards_celia.func_lib import get_res
...
def plot_res(net, t0, tmax, z0, zmax, steps):
    ...
    for it in range(len(lin_space_t)):
        for iz in range(len(lin_space_z)):
            t = torch.tensor(lin_space_t[it], dtype=richards_celia.DTYPE)
            z = torch.tensor(lin_space_z[iz], dtype=richards_celia.DTYPE)
            input = torch.tensor([[t, z]], requires_grad=True, dtype=richards_celia.DTYPE, device=richards_celia.DEVICE)
